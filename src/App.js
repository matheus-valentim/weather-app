	...
	async function teste() {
		...
		try {
			await axios.get(url).then((dados) => {
				setData(dados.data);
				setImagem(dados.data.current.condition.icon);
			});
		} catch (error) {
			console.log(error);
		}
			...
			{data ? (
				<div className="wrapper">
					<div className="div_h2">
						<h1 className="nome">{data.location.name}</h1>
					</div>
					<div className="img_temp">
						<div ClassName="div_imagem">
							{imagem ? (
								<img src={imagem} alt={data.current.condition.text}></img>
							) : (
								"nada"
							)}
						</div>
						<div className="div_temp">
							<h2 className="temp">{data.current.temp_c}°C</h2>
						</div>
					</div>
					<div className="div_condicao">
						<p className="condicao">{data.current.condition.text}</p>
					</div>
				</div>
			) : null}
		</div>
